  ...
  <div>
    ...
    <el-card style="border-radius: 10px">
      ...
      <el-row :gutter="25">
        ...
        <el-col :span="4">
          <el-button type="primary" @click="addDialogVisible=true">登记待缴水费</el-button>
        </el-col>
      </el-row>

      <!--用户列表 border边框 stripe隔行变色-->
      <el-table :data="fee_water" border stripe>
        <!--  索引列-->
        <el-table-column prop="waterid" width="40px">
        </el-table-column>
        ...
        <el-table-column label="水费月份" prop="paymonth" :formatter="setDate"></el-table-column>
        <el-table-column label="水表读数" prop="waternum"></el-table-column>
        <el-table-column label="水费金额" prop="paymoney"></el-table-column>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" content="修改" placement="top">
              <el-button type="primary" icon="el-icon-phone" size="mini"
            ...
            <el-tooltip class="item" effect="dark" content="删除" placement="top">
              <el-button type="danger" icon="el-icon-delete" size="mini"
                         @click="deleteUser(scope.row.waterid)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="登记待缴水费" :visible.sync="addDialogVisible" width="40%" @close="addDialogClosed()">
      <el-form :model="addForm"
        ...
        <el-form-item label="水费月份" prop="payMonth">
          <el-date-picker
              v-model="addForm.paymonth"
              type="month"
              placeholder="水费月份" style="width: 100%" value-format="yyyy-MM-dd">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="水表读数" prop="waterNum">
          <el-input v-model="addForm.waternum" clearable></el-input>
        </el-form-item>
        <el-form-item label="水费金额" prop="payMoney">
    ...
    <el-dialog title="登记待缴水费" :visible.sync="editDialogVisible" width="40%" @close="addDialogClosed()">
      <el-form :model="editForm"
        ...
        <el-form-item label="水表读数" prop="waterNum">
          <el-input v-model="editForm.waternum" clearable></el-input>
        </el-form-item>
        <el-form-item label="水费金额" prop="payMoney">
  ...
  methods: {
    ...
    addUser() {
      this.$refs.addFormRules.validate(async valid => {
        if (!valid) return;
        let {data: res} = await this.$http.post("feeWater", this.addForm);
        if (res !== "success") {
    ...
    editOwner() {
      this.$refs.addFormRules.validate(async valid => {
        if (!valid) return;
        let {data: res} = await this.$http.put("feeWater", this.editForm);
        if (res !== "success") {
          return this.$message.error("修改失败");
        }
        this.$message.success("修改成功")
    ...
    async deleteUser(id) {
      let {data: res} = await this.$http.delete("feeWater?id=" + id)
      if (res !== "success") {
    ...
    showEditDialog(res) {
      ...
      console.log(res)
      this.editForm.waterid = res.waterid,
      this.editForm.ownerid = res.ownerid,

      this.editForm.waternum = res.waternum,
