  ...
  <div>
    ...
    <el-card style="border-radius: 10px">
      ...
      <el-row :gutter="25">
        ...
        <el-col :span="4">
          <el-button type="primary" @click="addDialogVisible=true">登记待缴物业费</el-button>
        </el-col>
      </el-row>

      <!--用户列表 border边框 stripe隔行变色-->
      <el-table :data="fee_property" border stripe>
        <!--  索引列-->
        <el-table-column prop="propertyid" width="40px">
        </el-table-column>
        ...
        <el-table-column label="物业费月份" prop="paymonth" :formatter="setDate"></el-table-column>
        <el-table-column label="物业费金额" prop="paymoney"></el-table-column>
          ...
          <template slot-scope="scope">
            ...
            <el-tooltip class="item" effect="dark" content="删除" placement="top">
              <el-button type="danger" icon="el-icon-delete" size="mini"
                         @click="deleteUser(scope.row.propertyid)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="登记待缴物业费" :visible.sync="addDialogVisible" width="40%" @close="addDialogClosed()">
      <el-form :model="addForm"
        ...
        <el-form-item label="物业费月份" prop="payMonth">
          <el-date-picker
              v-model="addForm.paymonth"
              type="month"
              placeholder="物业费月份" style="width: 100%" value-format="yyyy-MM-dd">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="物业费金额" prop="payMoney">
    ...
    <el-dialog title="登记待缴物业费" :visible.sync="editDialogVisible" width="40%" @close="addDialogClosed()">
  ...
  methods: {
    ...
    addUser() {
      this.$refs.addFormRules.validate(async valid => {
        if (!valid) return;
        let {data: res} = await this.$http.post("feeProperty", this.addForm);
        if (res !== "success") {
    ...
    editOwner() {
      this.$refs.addFormRules.validate(async valid => {
        if (!valid) return;
        let {data: res} = await this.$http.put("feeProperty", this.editForm);
        if (res !== "success") {
          return this.$message.error("修改失败");
        }
        this.$message.success("修改成功")
    ...
    async deleteUser(id) {
      let {data: res} = await this.$http.delete("feeProperty?id=" + id)
      if (res !== "success") {
    ...
    showEditDialog(res) {
      ...
      console.log(res)
      this.editForm.propertyid = res.propertyid,
          this.editForm.ownerid = res.ownerid,
          this.editForm.propertynum = res.propertynum,
