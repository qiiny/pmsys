<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>社区交流管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 用户列表主体 -->
    <el-card style="border-radius: 10px">
      ...
      <el-row :gutter="25">
        <el-col :span="8">
          <el-input placeholder="请输入要搜索的内容" v-model="queryInfo.title" clearable @clear="getAllComplaint">
            <el-button slot="append" icon="el-icon-search" @click="getAllComplaint"></el-button>
          </el-input>
        </el-col>
      </el-row>
      ...
      <el-table :data="articleList" border stripe>
        <!--  索引列-->
        <el-table-column prop="complaintid" width="40px">
        </el-table-column>
        <el-table-column label="作者" prop="author" width="180px"></el-table-column>
        <el-table-column label="标题" prop="title" width="180px"></el-table-column>
        <el-table-column label="内容" prop="content"></el-table-column>
        <el-table-column label="发布时间" prop="createtime" :formatter="setDate" width="180px">
        </el-table-column>
        <el-table-column label="是否撤销" prop="comment" width="180px">
        </el-table-column>
    ...
    <el-dialog title="回复" :visible.sync="editDialogVisible" width="40%" @close="addDialogClosed()">
  ...
  methods: {
    ...
    addNotice() {
      this.$refs.addFormRules.validate(async valid => {
        if (!valid) return;
        let {data: res} = await this.$http.post("complaint", this.addForm);
        if (res !== "success") {
    ...
    editOwner() {
      this.$refs.addFormRules.validate(async valid => {
        if (!valid) return;
        let {data: res} = await this.$http.put("complaint", this.editForm);
        if (res !== "success") {
          return this.$message.error("修改失败");
        }
        this.$message.success("修改成功")
    ...
    async deleteUser(id) {
      let {data: res} = await this.$http.delete("complaint?id=" + id)
      if (res !== "success") {
    ...
    showEditDialog(res) {
      ...
      this.editForm.complaints=res.complaints
      this.editForm.complaintid=res.complaintid
